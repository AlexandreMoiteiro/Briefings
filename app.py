# app.py
# Flight Briefing — no sidebar, look clean
# RAW (EN): charts + link Weather + RAW GAMET (saved) + RAW SIGMET (auto LPPC)
...
def analyze_chart_pt(kind: str, img_b64: str) -> str:
    sys = ("És meteorologista aeronáutico sénior. Analisa o chart fornecido (PT), em prosa contínua, "
           ...
           "Sem listas. Sem invenções; usa só o visível.")
...
class RawPDF(FPDF):
    ...
    def cover(self, pilot, aircraft, callsign, reg, date_str, time_utc, mission):
        ...
        self.cell(0,8,f"Date: {date_str}   UTC: {time_utc}", ln=True, align="C"); self.ln(6)
        self.set_font("Arial","I",12); self.cell(0,8,f"Current METAR / TAF / SIGMET: {APP_WEATHER_URL}", ln=True, align="C")
        if mission: self.ln(4); self.set_font("Arial","",12); self.multi_cell(0,7,f"Remarks: {mission}", align="C")
    def gamet_raw(self, text):
        if not text.strip(): return
        self.add_page(orientation="P"); draw_header(self,"GAMET (RAW)")
        self.set_font("Arial","",12); self.ln(3); self.multi_cell(0,7,text)
    ...
    def chart_full(self, title, subtitle, img_png):
        self.add_page(orientation="L"); draw_header(self,title)
...
class DetailedPDF(FPDF):
    ...
    def cover(self, pilot, aircraft, callsign, reg, date_str, time_utc, mission):
        ...
        self.cell(0,8,f"Data: {date_str}   UTC: {time_utc}", ln=True, align="C"); self.ln(6)
        self.set_font("Arial","I",12); self.cell(0,8,f"METAR / TAF / SIGMET atualizados: {APP_WEATHER_URL}", ln=True, align="C")
        if mission: self.ln(4); self.set_font("Arial","",12); self.multi_cell(0,7,f"Notas: {mission}", align="C")
    def metar_taf_block(self, analyses: List[Tuple[str,str]]):
        ...
        for icao, text in analyses:
            ...
            self.set_font("Arial","",12); self.multi_cell(0,7,text); self.ln(2)
    def sigmet_block(self, sigmet_text: str, analysis_pt: str):
        ...
        self.ln(2); self.set_font("Arial","B",12); self.cell(0,8,"Texto (RAW):", ln=True)
        self.set_font("Arial","",12); self.multi_cell(0,7,sigmet_text); self.ln(4)
    ...
    def gamet_block(self, gamet_text: str, analysis_pt: str):
        ...
        self.ln(2); self.set_font("Arial","B",12); self.cell(0,8,"Texto (RAW):", ln=True)
        self.set_font("Arial","",12); self.multi_cell(0,7,gamet_text); self.ln(4)
        self.set_font("Arial","B",12); self.cell(0,8,"Interpretação:", ln=True)
        self.set_font("Arial","",12); self.multi_cell(0,7,analysis_pt)
...
colG1, colG2 = st.columns([0.2, 0.8])
with colG1:
...
with colG2:
    if saved_gamet.get("updated_utc"):
        st.markdown(f'<div class="info-line">Last saved (UTC): {saved_gamet["updated_utc"]}</div>', unsafe_allow_html=True)
