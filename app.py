...
def save_notams_to_gist(new_part: Dict[str, List[str]]) -> Tuple[bool, str]:
    ...
    try:
        ...
        payload = {"updated_utc": dt.datetime.utcnow().strftime("%Y-%m-%dT%H:%MZ"), "map": base_map}
        body = {"files": {fn: {"content": json.dumps(payload, ensure_ascii=False, indent=2)}}}
        r = requests.patch(
            f"https://api.github.com/gists/{gid}",
            headers={"Authorization": f"token {token}", "Accept": "application/vnd.github+json"},
...
class DetailedPDF(FPDF):
    ...
    def link_line(self, label: str, url: str) -> None:
        ...
        self.set_text_color(33, 111, 219)  # azul
        self.cell(0,7, ascii_safe(f"{label}: {url}"), ln=True, link=url)
        self.set_text_color(0,0,0)

    def metar_taf_block(self, analyses: List[Tuple[str, str, str, str]], weather_url: Optional[str] = None) -> None:
        self.add_page(orientation="P")
        draw_header(self, "METAR / TAF — Interpretacao (PT)")
        if weather_url:
            self.link_line("Weather page", weather_url)
        self.set_font("Helvetica","",12); self.ln(1)
...
class FinalBriefPDF(FPDF):
    ...
    def section_title_with_link(self, title: str, link_label: str, link_url: str) -> None:
        self.add_page(orientation="P")
        self.set_font("Helvetica","B",20)
        self.set_text_color(*PASTEL)
        self.cell(0,12,ascii_safe(title), ln=True, align="C")
        self.set_text_color(33,111,219)
        self.set_font("Helvetica","",11)
        self.cell(0,8, ascii_safe(f"{link_label}: {link_url}"), ln=True, align="C", link=link_url)
        self.set_text_color(0,0,0)

    def flightplan_image_portrait(self, title: str, img_png: io.BytesIO, show_notams_link: bool = False) -> None:
        self.add_page(orientation="P")
        draw_header(self, ascii_safe(title))
        if show_notams_link:
            self.set_font("Helvetica","",11); self.set_text_color(33,111,219)
            self.cell(0,7, ascii_safe(f"NOTAMs page: {APP_NOTAMS_URL}"), ln=True, link=APP_NOTAMS_URL, align="L")
            self.set_text_color(0,0,0)
...
with tab_notams:
    ...
    # Atualiza apenas o que editaste; o resto do Gist mantém-se
...
with tab_generate:
    ...
    st.markdown("#### Flight Plan & M&B uploads")
    fp_upload = st.file_uploader("Upload Flight Plan (PDF/PNG/JPG/JPEG/GIF)", type=["pdf","png","jpg","jpeg","gif"], key="fp_upload")
    mb_upload = st.file_uploader("Upload M&B/Performance (PDF)", type=["pdf"], key="mb_upload")
...
if 'gen_det' in locals() and gen_det:
    ...
    det_pdf.metar_taf_block(metar_analyses, weather_url=APP_WEATHER_URL)
    if sigmet_for_pdf:
...
if 'gen_final' in locals() and gen_final:
    ...
    # Flight Plan — se imagem, entra aqui já com link; se PDF, criamos página de secção com link e depois inserimos as páginas
    fp_obj = locals().get("fp_upload", None)
    fp_is_pdf = False
    fp_pdf_bytes: Optional[bytes] = None
    if fp_obj is not None:
        raw = fp_obj.read()
        if (fp_obj.type or "").lower() == "application/pdf":
            fp_is_pdf = True
            fp_pdf_bytes = raw
        else:
            fp_img_png = ensure_png_from_bytes(raw, fp_obj.type or "")
            fb.flightplan_image_portrait("Flight Plan", fp_img_png, show_notams_link=True)
    ...
    # Merge: se FP for PDF, inserir após criar página de secção com link; depois pares Navlog/VFR; depois M&B (sem links genéricos)
    pairs_local: List[Dict[str, Any]] = locals().get("pairs", [])
    mb_obj = locals().get("mb_upload", None)

    try:
        main = fitz.open(stream=fb_bytes, filetype="pdf")
        insert_pos = main.page_count  # inserir no fim

        # 1) Flight Plan PDF + link contextual de NOTAMs
        if fp_is_pdf and fp_pdf_bytes:
            # página de secção com link NOTAMs
            sec = FinalBriefPDF()
            sec.section_title_with_link("Flight Plan", "NOTAMs page", APP_NOTAMS_URL)
        ...
        # 2) Pares Navlog↔VFR (títulos livres)
        for p in (pairs_local or []):
            title_pair = p.get("title") or "Untitled"
            # título de secção do par (sem links)
            sec = FinalBriefPDF(); sec.section_title(f"Navlog & VFR — {title_pair}")
            sec_bytes = fpdf_to_bytes(sec)
            sec_doc = fitz.open(stream=sec_bytes, filetype="pdf")
            main.insert_pdf(sec_doc, start_at=insert_pos); insert_pos += sec_doc.page_count; sec_doc.close()
            ...
            if nv is not None:
                ...
                if (nv.type or "").lower() == "application/pdf":
                    ...
                    nv_bytes = image_bytes_to_pdf_bytes(f"Navlog — {title_pair}", img_png, orientation="P")
            ...
            if vf is not None:
                ...
                if (vf.type or "").lower() == "application/pdf":
                    ...
                    vf_bytes = image_bytes_to_pdf_bytes(f"VFR Map — {title_pair}", img_png, orientation="L")
                    vf_doc = fitz.open(stream=vf_bytes, filetype="pdf")
                    main.insert_pdf(vf_doc, start_at=insert_pos); insert_pos += vf_doc.page_count; vf_doc.close()

        # 3) M&B PDF (sem links genéricos)
        if mb_obj is not None:
            try:
                mb_bytes = mb_obj.getvalue() if hasattr(mb_obj, "getvalue") else mb_obj.read()
