...
# Raster / export defaults
SBS_DPI = 500
LANDING_BG_ZOOM = 2.3  # fixed — no UI control
...
def build_aerodromes_db(icaos):
    ...
    # ---- Overrides (porquê?)
    # OurAirports às vezes traz headings/sets de pistas desatualizados ou “a mais”.
    # Estes 2 overrides garantem os QFU/seleção exatamente como usas no teu SOP/folha.
    ...
    # - LPEV: remove 04/18; keep 01/19 and 07/25 with headings you want
    if "LPEV" in db:
        db["LPEV"]["name"] = "Évora"
        keep = {"01","19","07","25"}
...
def _preprocess_pdf_for_raster(pdf_bytes: bytes) -> bytes:
    # updates widget appearances before rasterizing
    try:
...
def pdf_two_pages_side_by_side_image(pdf_bytes: bytes, dpi: int = SBS_DPI, bg=(255, 255, 255)) -> Image.Image:
    ...
    # slight sharpen like your JPG app
...
def images_to_pdf_bytes(pages: List[Image.Image]) -> bytes:
    ...
    for img in pages:
        w, h = img.size
        c.setPageSize((w, h))  # 1px = 1pt (OK for internal use/print)
...
def pick_guides(cap: Dict[str, Any], mode: str) -> Tuple[List[Dict[str, float]], List[Dict[str, float]]]:
    ...
    # landing: prefer middle/right if present
    mid = g.get("middle", []) or []
    rgt = g.get("right", []) or []
...
# =========================
# Nicer overlay drawing for performance charts (NO % on charts)
# =========================
...
def draw_pill_label(img: Image.Image, tip_xy: Tuple[float, float], text: str, font_size: int = 18):
    """Nice label: dark translucent pill with white text. (No white box / black border.)"""
    ...
    # position near tip (try up-right then up-left)
    cand = [
        (tx + 14, ty - th - 18),
        (tx - tw - 14, ty - th - 18),
        (tx + 14, ty + 14),
...
def draw_chart_with_path(
    ...
    # title (top-left, small dark strip)
...
def composite_2x2(charts: List[Image.Image], page_title: str) -> Image.Image:
    """2x2 grid on a landscape canvas."""
    assert len(charts) == 4
    # unify sizes (use smallest to avoid upscaling)
...
with tab1:
    ...
    with c1:
        ...
        st.markdown("<div class='muted'>Se quiseres, depois ligo isto ao teu Gist (fleet) como antes.</div>", unsafe_allow_html=True)
...
# =========================
# 3) Weight & Fuel (no fleet gist here; you can re-add later)
# =========================
with tab3:
...
with tabP:
    ...
    with c3:
        st.caption("Valores no gráfico: só o valor (sem percentagens). Percentagens só no PDF (TODR/LDR).")
    ...
    if compute_perf and cap_to and cap_cl and cap_ld:
        ...
        if not wb:
            ...
            for i, leg in enumerate(st.session_state.legs):
                ...
                rw = best["rw"]
                hw = max(0.0, float(best["hw"]))  # only headwind for chart input
                oat = float(met["temp_c"])
                pa = pa_ft_from_elev_qnh(ad["elev_ft"], met["qnh_hpa"])

                # compute based on appropriate weights
    ...
    for i, leg in enumerate(st.session_state.legs):
        ...
        # (PDF will include %; UI can show just meters)
    ...
    if show_previews and cap_to and cap_cl and cap_ld:
        st.markdown("#### Preview charts (sem %)")
        ...
        for i, leg in enumerate(st.session_state.legs):
            ...
            # build 3 images for this leg
            to_segs = p["paths"]["to"]
            cl_segs = p["paths"]["cl"]
            ld_segs = p["paths"]["ld"]

            # label is ONLY value (no %)
...
with tab4:
    ...
    if st.button("Generate PDF", type="primary"):
        try:
            ...
            # MTOW/MLW (these exact field names exist in your PDF)
            ...
            for i, leg in enumerate(st.session_state.legs):
                ...
                # this PDF has a weird name for departure PA:
                if suf == "DEPARTURE":
                ...
                # PERFORMANCE fields (TODR / LDR / ROC) — with % in PDF fields
