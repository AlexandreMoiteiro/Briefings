...
# API key setup (uncomment when API ready)
...
st.title("SPC AI Analysis – Only Cropped Area (Portugal & Vicinity)")
st.markdown(
    """
    **Step 1:** Upload your Surface Pressure Chart (SPC).<br>
    **Step 2:** Use the cropping tool below to select only the area covering Portugal and nearby regions.<br>
...
if spc_file:
    ...
    cropped_img = st_cropper(
        ...
        instructions="Drag and resize the box to include just Portugal and neighboring airspace. The area inside the box is what will be analyzed."
    )
    st.image(cropped_img, caption="Cropped Area (to be analyzed by AI)", use_container_width=True)

    # Downscale for token/cost efficiency
