# Streamlit app – Tecnam P2008 (M&B + Performance) – v8.2 (Open-Meteo, rounding, no semicolons)
# Reqs: streamlit, requests, pypdf>=4.2.0, pytz
...
with tab_perf:
    ...
    RATE_LPH = st.number_input("Débito (L/h)", min_value=10.0, max_value=40.0, value=20.0, step=0.5)
    ...
    # Helpers
    def l_from_min(mins, rate=RATE_LPH): return round(rate * (mins/60.0), 1)
    def fmt_l(v): return f"{v:.1f} L" if abs(v-round(v))>1e-9 else f"{int(round(v))} L"
    ...
    # (10) Extra — SEMPRE ajustado para bater no M&B
    ...
    st.session_state["_fuel"] = {
        ...
        "alt_min": alt_min, "reserve_min": reserve_min, "cont_min": cont_min,
...
with tab_pdf:
    ...
    try:
        ...
        # Fuel (com " L")
        RATE_LPH = st.session_state.get("_fuel", {}).get("rate_lph", 20.0) if False else 20.0
        def L_from_min(m):
            return int(round(20.0 * ((m or 0)/60.0)))

        taxi_min = int(round(fuel.get("taxi_min", 0)))
        trip_min = int(round((fuel.get("trip_l",0)/20.0)*60))
        ...
        reserve_min = int(round(fuel.get("reserve_min", 45)))
        ...
        total_ramp_min = int(round(fuel.get("total_ramp_min", req_ramp_min)))
        ...
        put_any(named_map, fieldset, "Ramp_F", f"{req_ramp} L")
        put_any(named_map, fieldset, "Extra_T", fmt_hm(int(round((extra_l/20.0) * 60))))
        put_any(named_map, fieldset, "Extra_F", f"{extra_l} L")
        put_any(named_map, fieldset, "Total_T", fmt_hm(total_ramp_min))
        put_any(named_map, fieldset, "Total_F", f"{total_ramp} L")
