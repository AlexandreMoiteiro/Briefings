...
def pick_qfu_auto(icao: str, wind_dir: float | None) -> float:
    ...
    if wind_dir is None or wind_dir == 0.0:
        return cands[0]
    # headwind = V * cos(diff)
    best = max(cands, key=lambda q: cos(radians(((wind_dir - q + 180) % 360) - 180)))
    return best
...
# =========================
# Performance tables (AFM extracts) – distances em m; ROC ft/min
# =========================
TAKEOFF = {
...
if simple_policy:
    ...
    # Ajuste automático do EXTRA para "casar" com o combustível em M&B
...
if simple_policy:
    ...
    st.markdown(f"- **(7) Alternate**: {alt_min_eff} min → {time_to_liters(0, alt_min_eff):.1f} L")
    st.markdown(f"- **(8) Reserve**: {reserve_min_eff} min → {time_to_liters(0, reserve_min_eff):.1f} L")
...
# =========================
# PDF export — named fields Dep/Arr/Alt (repo-first, no upload)
# =========================
...
if template_bytes:
    # M&B
    put_any(named_map, fieldset, ["EmptyWeight_W","EMPTY_W"], f"{ew:.1f}")
    put_any(named_map, fieldset, ["EmptyWeight_A","EMPTY_A"], f"{(ew_moment/ew if ew>0 else 0.0):.3f}")
    put_any(named_map, fieldset, ["EmptyWeight_M","EMPTY_M"], f"{m_empty:.2f}")
    ...
    put_any(named_map, fieldset, ["TOTAL_M","TOTAL_MOMENT"], f"{total_moment:.2f}")
    put_any(named_map, fieldset, ["CG"], f"{cg:.3f}")
    put_any(named_map, fieldset, ["Aircraf_Reg","REG"], reg_input or "")
    put_any(named_map, fieldset, ["Date","DATE"], date_str)
    ...
    put_any(named_map, fieldset, ["Block9_T","BLOCK9_T"], fmt_hm(block9_min if simple_policy else 0))
    put_any(named_map, fieldset, ["Block9_F","BLOCK9_F"], f"{time_to_liters(0, (block9_min if simple_policy else 0)):.0f}L")
