...
# =========================
# Fixed Aircraft Data (only Tecnam P2008)
# =========================
ac = {
...
# =========================
# Performance Tables (from AFM extracts)
# Distances em metros; ROC em ft/min
# Keys: PA in ft; temps in °C {-25, 0, 25, 50}; also "ISA"
# =========================
TAKEOFF = {
...
def roc_interp(pa, temp, weight):
    # clamp weight between 550 and 650 and interpolate by weight
    w_lo, w_hi = 550.0, 650.0
    w = clamp(weight, w_lo, w_hi)
    def roc_for_w(w_):
        tab = ROC[int(w_)]
        pas = sorted(tab.keys())
    ...
    # linear weight blend across 550–650 using 550 & 650 via 600 as mid; we do piecewise
    if w <= 600:
        v_lo = roc_for_w(550)
        v_hi = roc_for_w(600)
        return interp1(w, 550, 600, v_lo, v_hi)
    else:
        v_lo = roc_for_w(600)
        v_hi = roc_for_w(650)
        return interp1(w, 600, 650, v_lo, v_hi)

# =========================
# Wind components & corrections
# =========================

def wind_head_component(runway_qfu_deg, wind_dir_deg, wind_speed):
    if runway_qfu_deg is None or wind_dir_deg is None:
        return 0.0
    # angle difference runway heading vs wind FROM direction
...
def to_corrections_takeoff(ground_roll, headwind_kt, paved=False, slope_pc=0.0):
    # Base corrections: Headwind -5 m/kt, Tailwind +15 m/kt; Paved -10% (GR); Slope +7% per +1%
    gr = ground_roll
    if headwind_kt >= 0:
...
def ldg_corrections(ground_roll, headwind_kt, paved=False, slope_pc=0.0):
    # Headwind -4 m/kt, Tailwind +13 m/kt; Paved -10%; Slope -3% per +1%
    gr = ground_roll
    if headwind_kt >= 0:
...
with left:
    ...
    # Limits & remaining fuel capacity
    ...
    def color_code_weight(val, limit):
        ...
        return 'ok'
    def color_code_cg(cg, limits):
        lo, hi = limits
        margin = 0.05*(hi-lo)
        if cg<lo or cg>hi: return 'bad'
        if cg<lo+margin or cg>hi-margin: return 'warn'
        return 'ok'
...
with right:
    ...
    # Defaults
    default_ad = [
        {"role":"Departure","icao":"LPSO","qfu":0.0,"elev_ft":390.0,"qnh":1013.0,"temp":15.0,"wind_dir":0.0,"wind_kt":0.0,"paved":False,"slope_pc":0.0,"toda_avail":0.0,"lda_avail":0.0},
        {"role":"Arrival","icao":"LPEV","qfu":0.0,"elev_ft":0.0,"qnh":1013.0,"temp":15.0,"wind_dir":0.0,"wind_kt":0.0,"paved":False,"slope_pc":0.0,"toda_avail":0.0,"lda_avail":0.0},
        {"role":"Alternate","icao":"LPCB","qfu":0.0,"elev_ft":0.0,"qnh":1013.0,"temp":15.0,"wind_dir":0.0,"wind_kt":0.0,"paved":False,"slope_pc":0.0,"toda_avail":0.0,"lda_avail":0.0},
    ]

    if 'aerodromes' not in st.session_state:
        st.session_state.aerodromes = default_ad

    perf_rows = []

    for i, a in enumerate(st.session_state.aerodromes):
        with st.expander(f"{a['role']} – {a['icao'] or ''}", expanded=(i==0)):
            ...
            # Interpolate distances at PA/temp
            ...
            # Corrections (apply to Ground Roll per AFM notes)
    ...
    # Show summary table
    def fmt(v, unit=""):
        return f"{v:.0f}{unit}" if isinstance(v, (int,float)) else str(v)

    st.markdown("#### Resumo de Performance")
    st.markdown("<table class='mb-table'><tr><th>Leg/Aeródromo</th><th>PA ft</th><th>DA ft</th><th>TO GR (m)*</th><th>TODR 50ft (m)</th><th>LND GR (m)*</th><th>LDR 50ft (m)</th><th>TODA Avail</th><th>LDA Avail</th></tr>" + 
                "".join([f"<tr><td>{r['role']} {r['icao']}</td><td>{fmt(r['pa_ft'])}</td><td>{fmt(r['da_ft'])}</td><td>{fmt(r['to_gr'],'')}</td><td>{fmt(r['to_50'],'')}</td><td>{fmt(r['ldg_gr'],'')}</td><td>{fmt(r['ldg_50'],'')}</td><td>{fmt(r['toda_av'])}</td><td>{fmt(r['lda_av'])}</td></tr>" for r in perf_rows]) +
...
def time_to_liters(hh=0, mm=0, rate=rate_lph):
    return rate * (hh + mm/60.0)
...
pdf_template_path = "TecnamP2008MBPerformanceSheet_MissionX.pdf"  # coloque o ficheiro na raiz da app

# Mapping de campos (pode precisar de ajustes conforme o PDF final)
FIELD_MAP = {
    # Page 1 (W&B)
    "Textbox19": reg,   # Aircraft Reg
    "Textbox18": date_str,  # Date
    # Totals/CG – vamos escrever mais abaixo com cores
}

# Campos página 2 – exemplo com nomes vistos no PDF de exemplo
# (Se algum nome divergir no teu PDF em branco, ajustamos.)

# Aeródromos (usamos o primeiro como exemplo para preencher; repetimos por role)
for r in perf_rows:
    role = r['role']
    if role == 'Departure':
        FIELD_MAP.update({
            "Textbox22": r['icao'],
            "Textbox53": f"{r['elev_ft'] if 'elev_ft' in r else ''}",
            "Textbox52": f"{st.session_state.aerodromes[0]['qnh']}",
            "Textbox51": f"{st.session_state.aerodromes[0]['temp']}",
            "Textbox50": f"{r['pa_ft']:.0f}",
            "Textbox49": f"{r['da_ft']:.0f}",
            "Textbox47": f"{int(r['toda_av'])}/{int(r['lda_av'])}",
            "Textbox45": f"{r['to_50']:.0f}",
            "Textbox41": f"{r['ldg_50']:.0f}",
            "Textbox39": f"{roc_interp(r['pa_ft'], st.session_state.aerodromes[0]['temp'], total_weight):.0f}",
        })

# Fuel planning – valores principais
FIELD_MAP.update({
    # Exemplos de campos do teu PDF; ajusta se necessário
    "Textbox60": f"{time_to_liters(0, su_min):.0f} L",
    "Textbox70": f"{time_to_liters(0, reserve_min):.0f} L",
    "Textbox68": f"{time_to_liters(0, alt_min):.0f} L",
    "Textbox66": f"{extra_l:.0f} L",
    "Textbox61": f"{total_ramp:.0f} L",
})
...
if st.button("Gerar PDF preenchido"):
    try:
        ...
        if not hasattr(reader, 'Root'):
            st.error("Não consegui abrir o template do PDF. Confirma o caminho do ficheiro.")
        else:
                ...
                def set_field(name, value, color_rgb=None):
                    for f in fields:
                        if f.get('/T') and f['/T'][1:-1] == name:
                            ...
                            if color_rgb:
                                r, g, b = color_rgb
                                f.update(PdfDict(DA=f"0 0 0 rg /Helv 10 Tf"))
                                f.update(PdfDict(DA=f"{r/255:.3f} {g/255:.3f} {b/255:.3f} rg /Helv 10 Tf"))
                            break
                for k, v in FIELD_MAP.items():
                    set_field(k, v)

                # Campos de cor (peso & CG) – nomes de exemplo; ajusta aos teus Textbox#
                wt_color = (30,150,30) if total_weight <= ac['max_takeoff_weight'] else (200,0,0)
                cg_lo, cg_hi = ac['cg_limits']
                if cg < cg_lo or cg > cg_hi:
                    cg_color = (200,0,0)
                else:
                    # margem 5%
                    margin = 0.05*(cg_hi-cg_lo)
                    if cg < cg_lo+margin or cg > cg_hi-margin:
                        cg_color = (200,150,30)
                    else:
                        cg_color = (30,150,30)
                set_field("Textbox17", f"{ac['max_takeoff_weight']:.0f}")
                set_field("Textbox14", f"{total_weight:.1f}", color_rgb=wt_color)
                set_field("Textbox16", f"{cg:.3f}", color_rgb=cg_color)
                set_field("Textbox5", f"{ac['cg_limits'][0]:.3f}")
                # (podes mapear mais campos de W&B conforme necessário)

            out_name = f"MB_Performance_{reg}_{mission}.pdf"
            writer = PdfWriter()
            writer.write(out_name, reader)
            st.success("PDF gerado com sucesso!")
            with open(out_name, 'rb') as f:
                st.download_button("Descarregar PDF", f, file_name=out_name, mime="application/pdf")
    except Exception as e:
        st.error(f"Erro a gerar o PDF: {e}")
