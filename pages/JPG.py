...
st.title("PDF (2 páginas) → Imagem lado a lado")

st.caption(
    "Converta PDFs de 2 páginas para uma única imagem. "
...
def convert_pdf_to_image(
    ...
    with fitz.open(stream=pdf_bytes, filetype="pdf") as doc:
        if doc.page_count < 2:
            raise ValueError("O PDF precisa de pelo menos 2 páginas.")
    ...
    for f in files:
        try:
            out_bytes, mime, ext, size = convert_pdf_to_image(
                f.read(), dpi=dpi, fmt=fmt, align_by=align_by, gap_px=gap_px, bg=BG, sharpen=sharpen
            )
            name = f.name.rsplit(".", 1)[0] + f"_merged." + ext
            w, h = size
            st.write(f"**{name}** — {w}×{h}px • {dpi} dpi • {fmt}")
            if preview_1to1:
                st.image(out_bytes)  # tamanho real
            else:
