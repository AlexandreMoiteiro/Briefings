...
# - Waypoints (nome, lat, lon, altitude, STOP, VOR fixo)
# - NÃO guarda vento nem tempos no Gist
# No dia de voo:
# - Colocas vento global, hora, EFOB, etc.
# - Carregas rota padrão
# - Carregas em "Gerar/Atualizar rota" → tudo calculado para esse dia
# ---------------------------------------------------------------
...
with st.form("globals"):
    ...
    with c4:
        st.session_state.start_efob= st.number_input("EFOB inicial (L)", 0.0, 200.0, float(st.session_state.start_efob), step=1.0)
...
if st.session_state.wps:
    with st.expander("📡 VOR em massa para vários WPs", expanded=False):
        st.caption("Escolhe um VOR e aplica a uma série de pontos. AUTO = VOR mais próximo.")
                    ...
                    for node in st.session_state.route_nodes:
                        for i, w in enumerate(st.session_state.wps):
                            if (
                                ...
                                break
                st.success(f"VOR aplicado: {applied_txt}. Volta a carregar em «Gerar/Atualizar rota» para refrescar tudo.")
