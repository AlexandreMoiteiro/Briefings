# app.py — NAVLOG (TOC/TOD só para CRUISE, nomes sem duplicar em sequência,
# altitudes DEP/ARR editáveis na tabela, HOLDs, PDF + Relatório, JSON v2)
...
def mmss_from_seconds(tsec: int) -> str:
    """<60 min -> mm:ss ; >=60 min -> hh:mm"""
    if tsec is None: tsec = 0
    tsec = int(max(0, tsec))
    if tsec < 3600:
        m = tsec // 60; s = tsec % 60
        return f"{m:02d}:{s:02d}"
    else:
        h = tsec // 3600; m = (tsec % 3600)//60
...
def rebuild_alt_rows(points: List[str], cruise:int, prev: Optional[List[dict]]):
    ...
    for i,p in enumerate(points):
        ...
        # por defeito, mas editável posteriormente na tabela:
        if i==0 and not base.get("Fix", False):
            row["Fix"]=True;  row["Alt_ft"]=float(dep_e)
        elif i==len(points)-1 and not base.get("Fix", False):
            row["Fix"]=True;  row["Alt_ft"]=float(arr_e)
        out.append(row)
    return out
        ...
        if isinstance(obj, pd.DataFrame):
            ...
            for _, row in obj.iterrows():
                ...
                for k,v in row.items():
                    if k in ("From","To","Point"): d[k]=clean_point_name(v)
...
#      + NOVO: Fix_from/Alt_from_ft aplicável à PRIMEIRA perna (DEP) para editar DEP na própria tabela
def build_combined_rows(points: List[str], legs: List[dict], alts: List[dict]) -> List[dict]:
    ...
    for i in range(1, len(points)):
        ...
        # origem (apenas para a primeira linha permitir editar DEP)
        a_from = alts[i-1] if i-1 < len(alts) else {"Fix": True, "Alt_ft": _round_alt(aero_elev(points[0]))}
        rows.append({
            "From": frm,
            "To": to,
            ...
            # destino
            "Fix_to": bool(a_to.get("Fix", False)) if i not in (len(points)-1,) else True,
            "Alt_to_ft": (float(a_to.get("Alt_ft", _round_alt(st.session_state.cruise_alt))) if bool(a_to.get("Fix", False)) or i==len(points)-1 else ""),
            "Hold_to": bool(a_to.get("Hold", False)),
            "Hold_to_min": float(a_to.get("Hold_min", 0.0)),
            # origem (só lido na 1.ª perna)
            "Fix_from": True if i==1 else False,
            "Alt_from_ft": float(a_from.get("Alt_ft", _round_alt(aero_elev(points[0])))) if i==1 else "",
        })
    return rows
...
def normalize_from_combined(points: List[str], edited_rows: List[dict]) -> Tuple[List[dict], List[dict]]:
    ...
    # alt_rows por índice
    dep_e = _round_alt(aero_elev(points[0]))
    arr_e = _round_alt(aero_elev(points[-1]))
    out = []
    for i, p in enumerate(points):
        base = {"Fix": False, "Alt_ft": float(_round_alt(st.session_state.cruise_alt)),
                "Hold": False, "Hold_min": 0.0}
        if i == 0:
            # ler da 1ª linha (Fix_from/Alt_from_ft) se fornecido; senão default à elevação
            r0 = edited_rows[0] if edited_rows else {}
            alt_from = r0.get("Alt_from_ft", "")
            if alt_from != "":
                base["Fix"] = True
                base["Alt_ft"] = float(alt_from)
            else:
                base["Fix"] = True
                base["Alt_ft"] = float(dep_e)
        elif i == len(points)-1:
            # ARR sempre "Fix", mas altitude pode ser alterada via última linha (Fix_to + Alt_to_ft)
            r_last = edited_rows[-1] if edited_rows else {}
            base["Fix"] = True
            alt_to_last = r_last.get("Alt_to_ft","")
            base["Alt_ft"] = float(alt_to_last) if alt_to_last!="" else float(arr_e)
            ...
            base["Fix"] = is_fix
            base["Alt_ft"] = (float(r.get("Alt_to_ft", _round_alt(st.session_state.cruise_alt))) if is_fix and r.get("Alt_to_ft","")!=""
...
# ===== Editor Único =====
st.subheader("Plano (TC/Dist + Altitude & HOLD no Fix de destino) — inclui DEP editável na 1ª linha")
combined_cfg = {
    ...
    # origem (só relevante na 1.ª linha, mas visível em todas)
    "Fix_from": st.column_config.CheckboxColumn("Fixar 'From' (só 1.ª linha)"),
    "Alt_from_ft": st.column_config.NumberColumn("Alt 'From' (ft) — DEP", step=50, min_value=0.0),
    # destino
...
start_alt = float(alts[0]["Alt_ft"]) if alts and "Alt_ft" in alts[0] else float(dep_elev)
...
for i,p in enumerate(points):
    ...
    if i==0:                A_target.append(float(r.get("Alt_ft", dep_elev)))   # DEP editável
    elif i==len(points)-1:  A_target.append(float(r.get("Alt_ft", arr_elev)))   # ARR editável
    else:
        A_target.append(float(r.get("Alt_ft", cruise_alt)) if bool(r.get("Fix", False)) else float(cruise_alt))

# ======= PERFIL POR PERNA — TOC/TOD apenas para CRUISE =======
rows=[]; seq_points=[]
...
# Controlo de nomes para evitar VACOR, VACOR (2) consecutivos
_last_label = None
_occ_counter: Dict[str,int] = {}

def _occ_label(name: str) -> str:
    """Evita duplicar fixes consecutivos (VACOR → VACOR), mas distingue regressos não-consecutivos (VACOR (2))."""
    global _last_label
...
def add_seg(phase, frm, to, i_leg, d_nm, tas, ff_lph, alt_start_ft, rate_fpm):
    ...
    rows.append({
        ...
        "Dist (nm)": fmt(d_nm,'dist'), "ETE": mmss_from_seconds(int(ete_sec)), "ETO": eto,
        "Burn (L)": fmt(burn_raw,'fuel'), "EFOB (L)": fmt(efob,'fuel')
    })

    label_to = _occ_label(to) if to == points[i_leg+1] else to
    seq_points.append({
...
def add_hold(point_name, minutes, alt_now):
    global clock, efob
    ...
    rows.append({
        ...
        "Dist (nm)":"", "ETE": mmss_from_seconds(int(ete_sec)), "ETO": eto,
...
for i in range(N):
    ...
    if alt_to > cur_alt + 1e-6:
        ...
        if cur_alt < CRZ <= alt_to and remain > 1e-9:
            ...
            if d_to_crz > 1e-9:
                ...
                cur_alt = add_seg("CLIMB", _occ_label(frm) if frm==points[i] else frm, name_toc, i, d_to_crz, vy_kt, ff_climb, cur_alt, roc)
    ...
    # ---------- DESCIDA ----------
        ...
        if abs(cur_alt - CRZ) < 1e-6 and alt_to < CRZ and remain > 1e-9:
            # calcula quanto CRUISE cabe antes da descida
            # tempo necessário para descer até alt_to:
...
# Holds por ocorrência (nome com sufixo (n) apenas quando não consecutivo)
holds_by_occ = {}
for p in seq_points:
    if p.get("phase")=="HOLD":
        nm = p.get("name","")
...
with cC:
    isa_dev = st.session_state.temp_c - isa_temp(pressure_alt(_round_alt(aero_elev(points[0])), st.session_state.qnh))
...
if fieldset:
    ...
    PAll(["FLT TIME","FLT_TIME","FLIGHT_TIME"], mmss_from_seconds(int(tot_ete_sec)))
    ...
    isa_dev_i = int(round(st.session_state.temp_c - isa_temp(pressure_alt(_round_alt(aero_elev(points[0])), st.session_state.qnh))))
    ...
    # ===== OBSERVAÇÕES — tempos + holds por ocorrência =====
    obs_lines = [
        f"Start-up & Taxi: {st.session_state.taxi_min} min @ 20 L/h → {fmt(fuel_taxi,'fuel')} L",
        f"Climb: {mmss_from_seconds(phase_secs['CLIMB'])} → {fmt(phase_burn['CLIMB'],'fuel')} L",
        f"Enroute (Cruise): {mmss_from_seconds(phase_secs['CRUISE'])} → {fmt(phase_burn['CRUISE'],'fuel')} L",
        f"Descent: {mmss_from_seconds(phase_secs['DESCENT'])} → {fmt(phase_burn['DESCENT'],'fuel')} L",
    ]
    if holds_by_occ:
        for fix_occ, agg in holds_by_occ.items():
            # mostra minutos (ou horas) conforme a regra
            obs_lines.append(f"HOLD @{fix_occ}: {mmss_from_seconds(agg['sec'])} → {fmt(agg['burn'],'fuel')} L")
        obs_lines.append(f"TOTAL Holding: {mmss_from_seconds(phase_secs['HOLD'])} → {fmt(phase_burn['HOLD'],'fuel')} L")
    P("OBSERVATIONS", "\n".join(obs_lines))
...
def build_report_pdf():
    ...
    resume = [
        ...
        ["Elev DEP/ARR/ALTN (ft)", f"{fmt(_round_alt(aero_elev(points[0])),'alt')} / {fmt(_round_alt(aero_elev(points[-1])),'alt')} / {fmt(_round_alt(aero_elev(st.session_state.altn)),'alt')}"],
        ...
        ["QNH / OAT / ISA dev", f"{int(st.session_state.qnh)} / {int(st.session_state.temp_c)} / {int(round(st.session_state.temp_c - isa_temp(pressure_alt(_round_alt(aero_elev(points[0])), st.session_state.qnh))))}"],
    ...
    for i, p in enumerate(seq_points):
        ...
        if seg.get("phase") == "HOLD":
            ...
            ff_use = _round_unit(ff_climb if seg['phase']=='CLIMB' else ff_descent if seg['phase']=='DESCENT' else ff_cruise)
            sign = '+' if seg['phase']=='CLIMB' else '-' if seg['phase']=='DESCENT' else '±0'
            rate = int(abs(seg.get('rate_fpm',0)))
            steps = [
                ...
                ["4) ETE", f"{mmss_from_seconds(seg.get('ete_sec',0))}"],
                ["5) Débito combustível", f"{ff_use} L/h"],
                ["6) Burn", f"{fmt(seg['burn'],'fuel')} L"],
                ["7) Perfil vertical", f"{prev['alt']}→{seg['alt']} ft @ {sign}{rate} ft/min"],
