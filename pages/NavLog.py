# app.py — NAVLOG v11 (AFM-only, UI organizada)
# — TAS/FF da AFM (climb/cruise/desc), correções ISA & peso
# — ROD automático por ângulo (default 3°)
# — TOC/TOD vira novo FIX, checkpoints reiniciam em cada segmento
# — Histórico no topo; editor da próxima perna em baixo
# — "Guardar perna" vs "Definir próxima perna" (sem gravar)

import streamlit as st, datetime as dt, math
from math import sin, asin, radians, degrees

st.set_page_config(page_title="NAVLOG v11 (AFM)", layout="wide", initial_sidebar_state="collapsed")
...
def cruise_lookup(pa,rpm,oat,weight):
    ...
    tas0,ff0 = val(CRUISE[p0]); tas1,ff1 = val(CRUISE[p1])
    ...
    tas *= (1.0 + 0.033*((650.0 - float(weight))/100.0))  # correção de peso
...
.badge{display:inline-block;padding:6px 12px;border-radius:999px;background:#155eef;color:#fff;font-weight:700}
...
# ========== cabeçalho ==========
st.title("NAVLOG — Performance v11 (AFM)")
...
if not st.session_state.legs:
    ...
    for i,leg in enumerate(st.session_state.legs, start=1):
        st.markdown(f"**Perna {i}** · TC {rint(leg['inputs']['TC'])}°T · {leg['inputs']['Dist']:.1f} nm · "
                    f"ETE {mmss(leg['ete_sec'])} · End Alt {int(leg['end_alt'])} ft · EFOB {leg['end_efob']:.1f} L")
        for j,seg in enumerate(leg['segments'], start=1):
            st.caption(f"Seg{j}: {seg['name']} · TH/MH {rang(seg['TH'])}T/{rang(seg['MH'])}M · "
                       f"GS/TAS {rint(seg['GS'])}/{rint(seg['TAS'])} kt · {mmss(seg['time'])} · "
                       f"{seg['dist']:.1f} nm · Burn {r10f(seg['burn']):.1f} L")

# ========== editor da perna (embaixo) ==========
...
Vy=vy_interp(pa0); ROC=roc_interp(pa0,st.session_state.oat)                      # climb AFM
...
ROD = max(100.0, GSde * 5.0 * (st.session_state.desc_angle/3.0))                # 3° ≈ GS*5
...
if profile=="CLIMB":
    ...
    segA={"tag":"CRUISE","name":"Level","TH":THr,"MH":MHr,"GS":GScr,"TAS":TAS_cru,"ff":FF_cru,"time":tA,"dist":Dist,"alt0":Alt0,"alt1":END_ALT}
...
c1,c2,c3,c4 = st.columns(4)
c1.metric("Alt ini→fim (ft)", f"{int(segA['alt0'])} → {int(segA['alt1'])}")
...
d1,d2 = st.columns(2)
d1.metric("Tempo", mmss(segA['time']))
d2.metric("Dist & Burn", f"{segA['dist']:.1f} nm · {r10f(segA['burn']):.1f} L")
...
if cpA:
    htmlA = "<div class='tl'><div class='bar'></div>" + "".join(
        [f\"<div class='tick' style='left:{(c['t']/segA['time'])*100:.2f}%;'></div>"
         f\"<div class='lbl' style='left:{(c['t']/segA['time'])*100:.2f}%;\">T+{c['min']}m<br/>{c['nm']} nm<br/>{c['eto']}<br/>EFOB {c['efob']:.1f}</div>\" for c in cpA]
    ) + "</div>"
...
if segB:
    ...
    st.markdown(f"<div class='sep'></div><div class='badge'>{label}</div> "
                f"<span class='small'>· {mmss(segA['time'])} · {segA['dist']:.1f} nm desde o início</span>"
                f"<div class='sep'></div>", unsafe_allow_html=True)
...
if segB:
    ...
    e1.metric("Alt ini→fim (ft)", f"{int(segB['alt0'])} → {int(segB['alt1'])}")
    e2.metric("TH/MH (°)", f"{rang(segB['TH'])}T / {rang(segB['MH'])}M")
    e3.metric("GS/TAS (kt)", f"{rint(segB['GS'])} / {rint(segB['TAS'])}")
    e4.metric("FF (L/h)", f"{rint(segB['ff'])}")
    f1,f2 = st.columns(2)
    f1.metric("Tempo", mmss(segB['time']))
    f2.metric("Dist & Burn", f"{segB['dist']:.1f} nm · {r10f(segB['burn']):.1f} L")
    ...
    if cpB:
        htmlB = "<div class='tl'><div class='bar'></div>" + "".join(
            [f\"<div class='tick' style='left:{(c['t']/segB['time'])*100:.2f}%;'></div>"
             f\"<div class='lbl' style='left:{(c['t']/segB['time'])*100:.2f}%;\">T+{c['min']}m<br/>{c['nm']} nm<br/>{c['eto']}<br/>EFOB {c['efob']:.1f}</div>\" for c in cpB]
        ) + "</div>"
...
st.markdown(f"**Totais** — ETE {hhmmss(TOT_SEC)} · Burn {TOT_BURN:.1f} L · **EFOB fim {EF_END:.1f} L**")
...
st.caption("Dica: 'Guardar' adiciona ao histórico. 'Definir próxima' só propaga Alt/EFOB para a próxima perna.")
