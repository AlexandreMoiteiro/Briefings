# app.py — NAVLOG v9 (AFM) — Cada leg é uma "caixinha"
# Planeamento por pernas com TOC/TOD como FIX. TAS/FF por tabelas AFM; ROD por ângulo; ROC por tabela.
# Gestão de legs: cada leg tem a sua caixa (editar/apagar), com propagação automática para as seguintes.
# UI: cartões, métricas claras, timeline sem sobreposição, TOC/TOD destacados.
# Nota: tabelas de performance (Tecnam P2008) são de exemplo/resumo.
...
# ============== CONFIG ==============
st.set_page_config(page_title="NAVLOG v9 (AFM) — Legs em caixinhas", layout="wide", initial_sidebar_state="collapsed")
...
ens("weight", 650.0)
ens("rpm_climb", 2250)
ens("rpm_cruise", 2100)  # pedido: 2100 por defeito
ens("rpm_desc", 1800)
ens("desc_angle", 3.0)
ens("start_clock", "")
ens("start_efob", 85.0)
ens("legs", [])  # lista de legs (cada uma é um dict com inputs)
ens("computed", [])  # resultados calculados por leg (segments, tempos, burns, carry-overs)

# ============== HEADER (PARÂMETROS GLOBAIS) ==============
st.title("NAVLOG — v9 (AFM) • Cada leg é uma caixinha")
...
with st.form("hdr"):
    ...
    if st.form_submit_button("Aplicar parâmetros globais"):
        pass

# ============== TIMELINE HTML (sem sobreposição) ==============
CSS = """
<style>
.card{
  border:1px solid #e7e7e9;border-radius:14px;padding:14px 16px;margin-bottom:12px;background:#fff;
...
.tl .tocdot, .tl .toddot{
  position:absolute;top:-6px;width:14px;height:14px;border-radius:50%;transform:translateX(-50%);
...
def timeline(seg, cps, start_label, end_label, toc_tod=None):
    # toc_tod: {"type":"TOC"|"TOD", "t": seconds from start}
    total = max(1, int(seg['time']))
    bars = []
    # base bar
    html = f"{CSS}<div class='tl'><div class='head'><div>{start_label}</div><div>GS {rint(seg['GS'])} kt · TAS {rint(seg['TAS'])} kt · FF {rint(seg['ff'])} L/h</div><div>{end_label}</div></div><div class='bar'></div>"
    # checkpoints (labels abaixo, não colidem com marcador principal)
    for cp in cps:
        ...
        bars.append(lbl)
    # TOC/TOD marker (acima, bolha destacada)
    if toc_tod is not None and 0 < toc_tod['t'] < total:
        pct = (toc_tod['t']/total)*100.0
        cls = 'tocdot' if toc_tod['type'] == 'TOC' else 'toddot'
        tt  = toc_tod['type']
        bars.append(f"<div class='{cls}' title='{tt}' style='left:{pct:.2f}%;'></div>")
    html += ''.join(bars) + "</div>"
    st.markdown(html, unsafe_allow_html=True)

# ============== CÁLCULOS DE SEGMENTOS ==============
def build_segments(tc, dist, alt0, alt1, wfrom, wkt, ck_min, params):
    """Calcula segmentos da perna e devolve dict com tudo para apresentação e carry."""
    ...
    Vy  = vy_interp(pa0)
    ROC = roc_interp(pa0, oat)  # ft/min equivalente
    TAS_climb = Vy
    ...
    if profile == "CLIMB":
        ...
        t_need = (alt0 - alt1) / max(ROD, 1e-6)  # min
...
# ============== PROPAGAÇÃO & CÁLCULO GLOBAL ==============
def recompute_all():
    """Recalcula todas as legs em cadeia, propagando ALT/EFOB/hora."""
    ...
    carry_alt = st.session_state.legs[0]['Alt0'] if st.session_state.legs else 0.0
    # se a primeira leg não tiver Alt0 definido, deixa como 0; cada leg tem o seu Alt0 explícito

    carry_efob = float(st.session_state.start_efob)
    clock = base_time

    for idx, leg in enumerate(st.session_state.legs):
        # usa Alt0 como está na leg (não forçamos carry), mas mostramos carry no cartão
        res = build_segments(
        ...
        cpB = res["ck_func"](segs[1], int(leg['CK']), (base1 + dt.timedelta(seconds=segs[0]['time'])) if base1 and len(segs)>1 else None,
                             max(0.0, r10f(EF0 - segs[0]['burn'])) if len(segs)>1 else None) if len(segs)>1 else []

        # atualiza carry para a próxima leg (com base no fim desta leg)
        ...
        carry_alt = segs[-1]['alt1']  # se quiseres usar em prefill da próxima criação
...
with topc1:
    if st.button("➕ Nova leg", type="primary", use_container_width=True):
        # prefill Alt0 com carry da última, Alt1 igual, resto por defeito
        if st.session_state.computed:
            pref = dict(Alt0=r10f(st.session_state.computed[-1]["carry_alt_after"]), Alt1=r10f(st.session_state.computed[-1]["carry_alt_after"]))
...
with topc2:
    st.caption("Cada leg aparece como uma caixinha abaixo. Edita valores e clica **Atualizar** nessa caixinha; o efeito propaga às seguintes. Podes **Apagar** qualquer leg.")

st.markdown("---")

# ============== RENDER DE CADA LEg (CAIXA) ==============
if not st.session_state.legs:
    st.info("Sem legs ainda. Clica **Nova leg** para começar.")
else:
    # recalcula no primeiro draw
    recompute_all()
    ...
    for i, leg in enumerate(st.session_state.legs):
        ...
        with st.container():
            ...
            with header_cols[1]:
                st.metric("ETE", hhmmss(comp["tot_sec"]))
            with header_cols[2]:
                st.metric("Burn (L)", f"{comp['tot_burn']:.1f}")
            with header_cols[3]:
                st.metric("EFOB pós-leg (L)", f"{comp['carry_efob_after']:.1f}")
            ...
            with k3:
                st.metric("TOC/TOD", ("TOC" if comp["toc_tod"] and comp["toc_tod"]["type"]=="TOC" else ("TOD" if comp["toc_tod"] else "—")))
            with k4:
                st.metric("Dist total (nm)", f"{sum(s['dist'] for s in comp['segments']):.1f}")
            ...
            # TOC/TOD divider quando existe 2º segmento
...
if st.session_state.computed:
    ...
    st.success(f"Resumo da rota — ETE {hhmmss(total_time)} · Burn total {total_burn:.1f} L · EFOB final {st.session_state.computed[-1]['carry_efob_after']:.1f} L")
