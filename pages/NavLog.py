    ...
    if not st.session_state.legs:
        ...
        for i,leg in enumerate(st.session_state.legs, start=1):
            with st.container():
                ...
                with cA:
                    if st.button(f"➕ Nova perna a partir da Perna {i}", key=f'use_leg_{i}'):
                        st.session_state.carry_alt=float(leg['end_alt'])
                        st.session_state.carry_efob=float(leg['end_efob'])
                        st.info(f"Próxima perna irá começar a {int(st.session_state.carry_alt)} ft · EFOB {st.session_state.carry_efob:.1f} L (a partir da Perna {i}).")
                with cB:
                    st.caption("Dica: podes continuar a editar acima; esta ação só define os valores iniciais.")
...
subtitle1 = f"GS {rint(segA['GS'])} kt · TAS {rint(segA['TAS'])} kt · FF {rint(segA['ff'])} L/h"
st.caption(subtitle1)
st.markdown("<div class='card'>", unsafe_allow_html=True)
...
col3.metric("Tempo",mmss(segA['time']))
col4.metric("Dist & Burn",f"{segA['dist']:.1f} nm · {r10f(segA['burn'])} L")
...
# header com horas, se existir start
if base1:
    st.caption(f"Início {base1.strftime('%H:%M')} · Fim {(base1+dt.timedelta(seconds=segA['time'])).strftime('%H:%M')}")
...
if segB:
    ...
    subtitle2 = f"GS {rint(segB['GS'])} kt · TAS {rint(segB['TAS'])} kt · FF {rint(segB['ff'])} L/h"
    st.caption(subtitle2)
    ...
    c3.metric("Tempo",mmss(segB['time']))
    c4.metric("Dist & Burn",f"{segB['dist']:.1f} nm · {r10f(segB['burn']):.1f} L")
    ...
    if base2:
        st.caption(f"Início {base2.strftime('%H:%M')} · Fim {(base2+dt.timedelta(seconds=segB['time'])).strftime('%H:%M')}")
