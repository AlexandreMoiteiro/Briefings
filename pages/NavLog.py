# app.py — NAVLOG v9.1 (AFM-only + checkpoints por segmento)
# Streamlit app compacta e consistente (indentação corrigida)
...
def mag_heading(th: float, var_deg: float, east_is_neg: bool = False) -> float:
...
ROC_ENROUTE: Dict[int, Dict[int, int]] = {
...
ROC_FACTOR = 0.90
VY_ENROUTE: Dict[int, int] = {0: 67, 2000: 67, 4000: 67, 6000: 67, 8000: 67, 10000: 67, 12000: 67, 14000: 67}
CRUISE: Dict[int, Dict[int, tuple]] = {
...
def isa_temp(pa_ft: float) -> float:
    return 15.0 - 2.0 * (pa_ft / 1000.0)


def pressure_alt(alt_ft: float, qnh: float) -> float:
    return float(alt_ft) + (1013.0 - float(qnh)) * 30.0
...
def cruise_lookup(pa_ft: float, rpm: int, oat_c: Optional[float], weight_kg: float):
    rpm = min(int(rpm), 2265)  # restrição AFM
    ...
    # Peso (−100 kg → +3.3% KTAS)
    delta_w = 650.0 - float(weight_kg)
    tas *= 1.0 + 0.033 * (delta_w / 100.0)
    return max(0.0, tas), max(0.0, ff)


def roc_interp(pa: float, temp_c: float) -> float:
    pas = sorted(ROC_ENROUTE.keys())
    ...
    v1 = interp1(pa_c, p0, p1, v10, v11)
    return max(1.0, interp1(pa_c, p0, p1, v0, v1) * ROC_FACTOR)


def vy_interp(pa: float) -> float:
    pas = sorted(VY_ENROUTE.keys())
    pa_c = clamp(pa, pas[0], pas[-1])
    p0 = max([p for p in pas if p <= pa_c])
    p1 = min([p for p in pas if p >= pa_c])
...
ensure("rpm_cruise", 2100)  # default pedido
ensure("rpm_descent", 1800)
...
ROD = max(100.0, GSd * 5.0 * (st.session_state.descent_angle / 3.0))  # ~GS*5 para 3°
...
if profile == "CLIMB":
    ...
    segA = {
        ...
        "alt0": Alt0,
        "alt1": END_ALT,
        "burn": FF_cruise * (tA / 3600.0),
    }

segs: List[Dict] = [segA] + ([segB] if segB else [])
TOT_SEC = sum(int(s["time"]) for s in segs)
TOT_BURN = r1(sum(float(s["burn"]) for s in segs))

# ---------- Checkpoints ----------

def build_cps(seg: Dict, every_min: int, base_clock: Optional[dt.datetime], efob_start: float) -> List[Dict]:
    rows: List[Dict] = []
    t = 0
    while t + every_min * 60 <= seg["time"]:
        ...
        efob = max(0.0, r1(efob_start - burn))
...
def timeline(seg: Dict, cps: List[Dict], start_label: str, end_label: str) -> None:
...
# ---------- Clocks ----------
clock: Optional[dt.datetime] = None
...
if profile == "CLIMB":
    c5.metric("ROC @ início (ft/min)", rint(ROC))
else:
    c5.metric("ROD calc (ft/min)", rint(ROD))

b1, b2, b3 = st.columns(3)
b1.metric("Tempo", mmss(int(segA["time"])))
b2.metric("Dist (nm)", f"{segA['dist']:.1f}")
b3.metric("Burn (L)", f"{r1(segA['burn']):.1f}")
...
# TOC/TOD bem demarcado
if segB:
    toc_time = (base1 + dt.timedelta(seconds=int(segA["time"]))).strftime("%H:%M") if base1 else mmss(int(segA["time"]))
...
if segB:
    ...
    d2.metric("TH/MH (°)", f"{rang(segB['TH'])}T / {rang(segB['MH'])}M")
    d3.metric("GS/TAS (kt)", f"{rint(segB['GS'])} / {rint(segB['TAS'])}")
    d4.metric("FF (L/h)", f"{rint(segB['ff'])}")
    e1, e2, e3 = st.columns(3)
    e1.metric("Tempo", mmss(int(segB["time"])))
    e2.metric("Dist (nm)", f"{segB['dist']:.1f}")
    e3.metric("Burn (L)", f"{r1(segB['burn']):.1f}")
    EF1 = max(0.0, r1(EF0 - segA["burn"]))
...
EF_END = max(0.0, r1(float(st.session_state.carry_efob) - sum(s["burn"] for s in segs)))
st.markdown(f"**EFOB** — Start {float(st.session_state.carry_efob):.1f} L → End {EF_END:.1f} L")
st.markdown(f"**Altitude final para próxima perna:** {int(round(segB['alt1'] if segB else segA['alt1']))} ft")
...
if st.button("➕ Construir próxima perna (guardar esta)", type="primary"):
    st.session_state.legs.append({
        ...
        "Alt0": Alt0,
        "Alt1": Alt1,
        "wind": f"{W_from:03d}/{W_kt:02d}",
        "segments": segs,
        "tot_sec": TOT_SEC,
        "tot_burn": TOT_BURN,
    })
    st.session_state.carry_alt = segB['alt1'] if segB else segA['alt1']
    st.session_state.carry_efob = EF_END
    st.toast("Perna adicionada. Próxima perna pronta.")

# ---------- Histórico (vai ficando) ----------
st.subheader("Pernas construídas")
if st.session_state.legs:
    for idx, L in enumerate(st.session_state.legs, start=1):
        st.markdown(
            f"**Perna {idx}** — TC {L['TC']:.1f}°T · Dist {L['Dist']:.1f} nm · Vento {L['wind']} · ETE {mmss(L['tot_sec'])} · Burn {L['tot_burn']:.1f} L"
        )
        for j, s in enumerate(L["segments"], start=1):
            st.caption(
                f"Seg {j}: {s['name']} — TH/MH {rang(s['TH'])}T/{rang(s['MH'])}M · GS/TAS {rint(s['GS'])}/{rint(s['TAS'])} kt · {mmss(int(s['time']))}, {s['dist']:.1f} nm, {r1(s['burn']):.1f} L"
            )
else:
