...
def make_dme_arc_points(vor_ident: str, radius_nm: float, start_radial: float, end_radial: float, direction: str, step_deg: float, alt_ft: float, prefix: str = "ARC") -> Tuple[List[Point], str]:
    ...
    radials = arc_radials(start_radial, end_radial, direction, step_deg)
    points: List[Point] = []
    base = clean_code(prefix) or f"{vor['ident']}ARC"
    for radial in radials:
        lat, lon = dest_point(vor["lat"], vor["lon"], radial, radius)
        code = f"{base}{int(round(radial)) % 360:03d}"[:12]
        name = f"{vor['ident']} DME {radius:g} R{int(round(radial)) % 360:03d}"
        points.append(
            Point(
                code=code,
                name=name,
                ...
                alt=float(alt_ft),
                src="DMEARC",
                remarks=f"{format_vor_id(vor)} ARC {radius:g} NM {direction.upper()}",
                vor_pref="FIXED",
                vor_ident=vor["ident"],
                uid=next_uid(),
            )
        )
    return points, f"Arco {vor['ident']} DME {radius:g} NM: {len(points)} pontos ({int(round(radials[0])) % 360:03d}→{int(round(radials[-1])) % 360:03d})."
...
def make_base_map() -> folium.Map:
    ...
    token = str(st.session_state.get("openaip_token", "")).strip()
...
def render_route_map(wps: List[Dict[str, Any]], nodes: List[Dict[str, Any]], legs: List[Dict[str, Any]], *, key: str = "mainmap") -> Dict[str, Any]:
    ...
    # Airways layer from CSV. Empty selected_airways means: show all.
    if bool(st.session_state.show_airways) and not AIRWAYS_DF.empty:
        selected = set(st.session_state.selected_airways or [])
        for airway, grp in AIRWAYS_DF.groupby("airway"):
            if selected and airway not in selected:
                continue
...
ss("selected_airways", list_airways())
ss("openaip_token", os.getenv("OPENAIP_KEY", os.getenv("OPENAIP_API_KEY", "")))
ss("show_openaip", bool(st.session_state.get("openaip_token", "")))
...
with tab_route:
    ...
    with left:
        ...
        arc_c1, arc_c2, arc_c3, arc_c4, arc_c5, arc_c6 = st.columns([0.9, 0.8, 0.8, 0.8, 0.8, 0.9])
        with arc_c1:
        ...
        with arc_c6:
            arc_step = st.number_input("Step °", 1.0, 30.0, 10.0, step=1.0, key="arc_step")
        arc_c7, arc_c8, arc_c9 = st.columns([1, 1, 2])
        with arc_c7:
            arc_alt = st.number_input("Alt arco", 0.0, 45000.0, float(st.session_state.default_alt), step=100.0, key="arc_alt")
        with arc_c8:
            arc_prefix = st.text_input("Prefixo", "ARC", key="arc_prefix")
        with arc_c9:
            st.markdown("<div style='height:1.75rem'></div>", unsafe_allow_html=True)
            if st.button("Adicionar arco DME à rota", use_container_width=True):
                pts, msg = make_dme_arc_points(str(arc_vor), float(arc_radius), float(arc_start), float(arc_end), str(arc_dir), float(arc_step), float(arc_alt), str(arc_prefix))
                if not pts:
                    st.error(msg)
                else:
                    st.session_state.wps.extend([p.to_dict() for p in pts])
...
with tab_map:
    ...
    with top[4]:
        st.text_input("openAIP API key", key="openaip_token", type="password", help="Também pode vir de OPENAIP_KEY ou OPENAIP_API_KEY.")
    st.slider("Opacidade openAIP", 0.0, 1.0, key="openaip_opacity", step=0.05)
    with st.expander("Filtrar airways (por defeito mostra todas)", expanded=False):
        st.multiselect("Airways", list_airways(), key="selected_airways")
